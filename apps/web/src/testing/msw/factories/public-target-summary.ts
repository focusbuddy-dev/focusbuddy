...
/**
 * Role: Builds testing-only summary payloads for MSW web client tests.
 * Boundary: Testing support only. Must not be imported by production runtime code.
 * Ref: #179
 */
export function buildMockPublicTargetSummary(targetId: string): PublicTargetSummary {
  return {
    target: {
    ...
    recentPublicSessions: [
      {
        ...
        publicNote: 'Mocked response from the web test seam.',
      },
    ],
  };
}
