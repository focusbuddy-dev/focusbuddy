...
export function inspectGitWorktreeContext(cwd) {
  ...
  const currentEntry = entries.find(
    (entry) => path.resolve(entry.worktree) === normalizedTopLevel,
  );
  const branchRef = branch ? `refs/heads/${branch}` : "";
  const branchOwners = branchRef
    ? entries.filter((entry) => entry.branch === branchRef)
    : [];
  const reasons = [];
  ...
  if (currentEntry && branchRef && currentEntry.branch !== branchRef) {
    reasons.push(
      "current worktree の branch 情報と git branch --show-current が一致しません",
    );
  }
...
export function extractPatchTargetPaths(text) {
  const matches = normalizeGuardText(text).matchAll(
...
export function collectTargetGitContexts(event) {
  ...
  for (const targetPath of paths) {
    ...
    const inspectPath = stat.isDirectory()
      ? resolvedPath
    ...
    if (isSymlink) {
      ...
      const normalizedTopLevel = topLevel.endsWith(path.sep)
        ? topLevel
        : topLevel + path.sep;
